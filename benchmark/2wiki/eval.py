...
def calculate_metrics(retrieved_chunks: List[str], supporting_facts: List[List[str]], k_values: List[int] = [1, 3, 5, 10]) -> Dict[str, float]:
    """
    ...
    for k in k_values:
        if k <= len(retrieved_chunks):
            ...
            recall_k = relevant_in_k / len(supporting_strings) if supporting_strings else 0
...
async def evaluate_single_question(index: HiRAG, question_data: Dict[str, Any]) -> Dict[str, Any]:
    """
...
async def evaluate(benchmark_file: str, n_questions: int = 100, max_concurrent: int = 5):
    """
    ...
    for metric, values in aggregate_metrics.items():
        ...
        mean_metrics[f'std_{metric}'] = statistics.stdev(values) if len(values) > 1 else 0
    ...
    for k in k_values:
        if f'mean_recall@{k}' in mean_metrics:
            print(f"  Recall@{k}: {mean_metrics[f'mean_recall@{k}']:.3f} (±{mean_metrics[f'std_recall@{k}']:.3f})")
            print(f"  Precision@{k}: {mean_metrics[f'mean_precision@{k}']:.3f} (±{mean_metrics[f'std_precision@{k}']:.3f})")
