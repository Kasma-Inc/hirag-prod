...
class TestDotsTreeGeneration:
    ...
    def test_generate_tree_with_json_file(self):
        ...
        for element in tree:
            ...
            assert element['type'] in ['Title', 'Section-header'], \
            ...
            if element['text'].startswith('#'):
                ...
                assert element['level'] == expected_level, \
    ...
    def test_generate_tree_with_sample_data(self):
        ...
        assert len(main_title['children']) == 1, f"Expected 1 child, got {len(main_title['children'])}"
        ...
        assert len(subtitle['children']) == 1, f"Expected 1 grandchild, got {len(subtitle['children'])}"
        ...
        print(f"Tree structure: {main_title['text']} -> {subtitle['text']} -> {sub_subtitle['text']}")
