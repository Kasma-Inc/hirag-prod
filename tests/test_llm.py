...
class TestAPIConfig:
    ...
    def test_api_config_missing_api_key(self):
        ...
        with patch.dict(os.environ, {'TEST_BASE_URL': 'https://test.example.com'}, clear=True):
            with pytest.raises(ValueError, match="TEST_API_KEY environment variable is not set"):
    ...
    def test_api_config_missing_base_url(self):
        ...
        with patch.dict(os.environ, {'TEST_API_KEY': 'test-key-123'}, clear=True):
            with pytest.raises(ValueError, match="TEST_BASE_URL environment variable is not set"):
                APIConfig.from_env('TEST_API_KEY', 'TEST_BASE_URL')
...
class TestChatCompletion:
    ...
    async def test_chat_completion_with_system_prompt(self, chat_service):
        ...
        try:
            ...
            assert isinstance(response, str)
            assert len(response.strip()) > 0
            print(f"✅ Chat completion with system prompt successful: {response.strip()}")
...
class TestEmbeddingService:
    ...
    async def test_large_batch_embeddings(self, embedding_service):
        ...
        try:
            ...
            duration = end_time - start_time
            print(f"✅ Large batch embeddings successful in {duration:.2f}s, shape: {embeddings.shape}")
    ...
    def test_embedding_text_validation(self, embedding_service):
        ...
        with pytest.raises(ValueError, match="Text at index 0 is empty after stripping"):
            embedding_service._text_validator.validate_and_clean(["   "])
...
class TestEnvironmentConfiguration:
    ...
    def test_api_endpoints_format(self):
        ...
        for var_name, url in base_urls:
            if url:
                assert url.startswith('http'), f"{var_name} should start with http/https"
                assert not url.endswith('/'), f"{var_name} should not end with trailing slash"
...
if __name__ == "__main__":
    ...
    async def run_async_tests():
        ...
        embed_service = EmbeddingService(default_batch_size=10)  # Create service directly
