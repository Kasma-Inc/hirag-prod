...
class DotsRecursiveChunker:
    ...
    def chunk(self, items: Optional[List[Item]], header_set: Optional[set[str]]) -> List[DotsDenseChunk]:
        if not items:
            return []
        ...
        while i < len(items):
            ...
            while i < len(items):
                cur_item = items[i]
                if cur_item.documentKey in header_set or self._is_table(cur_item.chunkType):
                    break
