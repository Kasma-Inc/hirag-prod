...
class VanillaEntity(BaseEntity):
    ...
    async def _extract_entities_from_chunk(self, chunk: Chunk) -> List[Entity]:
        ...
        try:
            ...
            entity_result, history = await self._perform_initial_entity_extraction(chunk)
    ...
    async def _perform_initial_entity_extraction(self, chunk: Chunk) -> Tuple[str, List]:
        """
    ...
    async def _perform_entity_gleaning(
        self, entity_result: str, history: List
    ) -> str:
        """
    ...
    async def _should_continue_extraction(
        ...
        normalized_response = (
            termination_response.strip().strip('"').strip("'").lower()
        )
    ...
    async def _merge_duplicate_entities(self, entities: List[Entity]) -> List[Entity]:
        ...
        unique_entities = [
            ...
            if entity_counts[entity.page_content] == 1
        ]
        ...
        merged_entities = await _limited_gather(merge_tasks, self.entity_merge_concurrency)
    ...
    async def relation(
        ...
        """
        logging.info(f"[Relation] Starting extraction from {len(chunks)} chunks with {len(entities)} entities")
    ...
    async def _extract_relations_from_chunk(
        ...
        try:
            ...
            if not chunk_entities:
                logging.info(f"[Relation] No entities found for chunk {chunk.id}, skipping")
                return []
            ...
            relation_result = await self._perform_relation_gleaning(relation_result, history)
