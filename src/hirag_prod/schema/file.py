...
def create_file(
    ...
    **kwargs
    ...
    required_fields = ['documentKey', 'knowledgeBaseId', 'workspaceId', 'text', 'fileName']
    ...
    if 'type' in file_data and file_data['type'] is not None:
        valid_types = ["pdf", "docx", "pptx", "xlsx", "jpg", "png", "zip", "txt", "csv", "text", "tsv", "html", "md"]
        if file_data['type'] not in valid_types:
            raise ValueError(f"Invalid file type '{file_data['type']}'. Must be one of: {valid_types}")
