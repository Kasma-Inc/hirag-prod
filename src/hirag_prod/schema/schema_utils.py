...
import sys
from typing import get_origin, get_args, List, Optional, Union, Dict, Any
from datetime import datetime
from pydantic import BaseModel
from pydantic.fields import FieldInfo

def pydantic_to_pyarrow_type(field_type: type, field_info: FieldInfo = None) -> pa.DataType:
    ...
    if origin is list or origin is List:
        if args:
            ...
            elif item_origin is dict or hasattr(item_type, '__origin__') and str(item_type).startswith('dict'):
    ...
    if origin is dict or (hasattr(field_type, '__origin__') and str(field_type).startswith('dict')):
