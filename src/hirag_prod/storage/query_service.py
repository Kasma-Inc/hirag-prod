...
class QueryService:
    ...
    async def dual_recall_with_pagerank(
        ...
        if triplet_recall["relations"]:
            ...
            async def _fetch_entity_chunk_count(ent: str, workspace_id: str, knowledge_base_id: str) -> int:
                try:
                    node = await self.storage.vdb.query_node(ent, workspace_id, knowledge_base_id)
                    chunk_ids: List[str] = []
            ...
            counts = await asyncio.gather(
                *[_fetch_entity_chunk_count(eid, workspace_id, knowledge_base_id) for eid in query_entity_ids]
            )
            ent_to_chunk_count = {
