...
class PGVector(BaseVDB):
    ...
    # query the database for the top records similar to the query text
    async def query(
        self,
        query: str,
        workspace_id: str,
        ...
        async with AsyncSession(self.engine, expire_on_commit=False) as session:
            ...
            if rerank and scored:
                scored = await self.strategy_provider.rerank_chunk_query(
                    scored, query, topn
                )
    ...
    # retrieve records matching specific keys
    async def query_by_keys(
        ...
        async with AsyncSession(self.engine, expire_on_commit=False) as session:
            if not hasattr(model, key_column):
                raise ValueError(
                    f"Key column '{key_column}' not found on table '{table_name}'"
                )

            stmt = select(model).where(getattr(model, key_column).in_(key_value))
            if workspace_id and hasattr(model, "workspaceId"):
                stmt = stmt.where(model.workspaceId == workspace_id)
            if knowledge_base_id and hasattr(model, "knowledgeBaseId"):
                stmt = stmt.where(model.knowledgeBaseId == knowledge_base_id)
