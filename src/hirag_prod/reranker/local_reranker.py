...
class LocalReranker(Reranker):
    """Local deployment reranker using HTTP API - reads config from environment"""
    ...
    def __init__(
        ...
        """
        Initialize local reranker - config from environment variables
        """
        super().__init__(return_score)
        
        # Get config from environment variables (from .env file)
        self.base_url = (base_url or os.getenv("RERANKER_MODEL_BASE_URL")).rstrip("/")
        self.model_name = model_name or os.getenv("RERANKER_MODEL_NAME", "Qwen3-Reranker-8B")
        self.entry_point = entry_point or os.getenv("RERANKER_MODEL_Entry_Point", "/rerank")
        # Content-Type 由 headers 固定为 application/json，不用环境变量
        ...
        if not self.auth_token:
            raise ValueError("RERANKER_MODEL_Authorization environment variable is required")

    def _call_api_sync(self, query: str, documents: List[str]) -> List[dict]:
        """Call the local reranker API synchronously"""
        headers = {
            "Content-Type": "application/json",
            "Model-Name": self.model_name,
            "Entry-Point": self.entry_point,
            "Authorization": self.auth_token if self.auth_token.startswith("Bearer ") else f"Bearer {self.auth_token}",
        }
        ...
        print(f"🔍 DEBUG - Reranker API Request:")
        print(f"   URL: {url}")
    ...
    def _rerank(self, result_set: pa.Table, query: str) -> pa.Table:
        """Core reranking logic - no fallback, fails if API fails"""
        ...
        # Call API - no fallback, let it fail if API fails
        ...
        if not indices:
            return result_set.slice(0, 0)  # Empty result
        ...
        self.logger.info(f"Successfully reranked {len(docs)} documents to {len(result_set)} results")
        return result_set
